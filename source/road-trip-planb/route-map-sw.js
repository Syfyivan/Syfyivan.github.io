const CACHE_NAME = 'road-trip-planb-map-v17';
const CORE_PATHS = [
  './',
  './index.html',
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(keys.filter(key => key.startsWith('road-trip-planb-map-') && key !== CACHE_NAME).map(key => caches.delete(key))))
...
self.addEventListener('fetch', event => {
  ...
  if (url.origin !== self.location.origin || !url.pathname.includes('/road-trip-planb/')) return;
