(function () {
  // Plan B 统一费用数据（29天，川进滇出，10/4出发，最晚11/1回京）：门票、区间车、停车、接驳、油费与高速费，
  ...
  // 进川转场 D1—D4 全程高速；D5 雅康高速段收费；进出藏主体走 G318/稻城公路/G214 国道不计费；
  // D15/D18 林拉高速；D24 香丽高速段；回程 D25 丽攀、D26 雅西、D27—D29 京昆/京港澳。
  const EXPRESSWAY_DAYS = new Set([1, 2, 3, 4, 5, 15, 18, 24, 25, 26, 27, 28, 29]);
  ...
  const TICKETS = {
    ...
    8: [t('稻城亚丁门票+景区观光车（长线，抢停线前；2026观光车已免费，门票旺季146/人）', 'shuttle', [800, 1200]), t('洛绒牛场电瓶车（按需，可步行）', 'shuttle', [0, 320])],
    13: [t('然乌湖 / 米堆冰川方向门票或景交（视天气与道路，可不进）', 'shuttle', [0, 400])],
    22: [t('飞来寺梅里雪山观景台门票（看天，日照金山不作硬目标）', 'ticket', [0, 320])],
    23: [t('普达措国家公园门票+区间车（择机，或改独克宗古城/松赞林寺）', 'shuttle', [0, 900])],
    24: [t('虎跳峡上虎跳观景台门票+桥头停车（只观景，不走高路徒步）', 'ticket', [120, 320])]
  };
