...
function applySeasonalElderFriction(log, stepLabel, season, xun, picked) {
    ...
    return {
      ...
      settle: function (log) {
        ...
        if ((route.indexOf('路径二') === 0 || route.indexOf('受雇') === 0) && season.id === 'winter' && xun === 2) {
          if (picked.f_route_wage_winter_reply || picked.f_route_wage_winter_gift || picked.f_route_wage_split || picked.f_route_send || picked.f_social || picked.f_rest) {
    ...
    return {
      ...
      settle: function (log) {
        ...
        applySeasonalHouseholdFriction(log, stepLabel, season, xun, picked, {
          ...
          autumn: {
            handledIds: ['h_shop_collect', 'h_proxy', 'h_pay', 'h_clan', 'h_lease_city', 'h_side', 'h_autumn_reply'],
            doneTag: '秋后细账已拆',
  ...
  function stageMerchantHousehold() {
    ...
    return {
      ...
      settle: function (log) {
        ...
        if (season.id === 'spring' && xun === 2) {
          if (picked.h_collect || picked.h_trust_field || picked.h_literate || picked.h_clan || picked.h_wharf || picked.h_spring_deed || picked.h_spring_incense) {
        ...
        if (season.id === 'spring' && xun === 2) {
          if (picked.h_spring_incense || picked.h_spring_deed || picked.h_trust_field || picked.h_clan || picked.h_wharf) {
  ...
  function elderRoutePack() {
    ...
    if (isFarmRouteState() || isWageRouteState()) {
      ...
      pack.note = '举业一路到了晚年，看的是名色留下多少实际照应：生员能减一层外流，笔墨底子则更容易换来教馆、抄写和体面；更磨人的，是春头馆契、旧馆回话、伏夏馆汤、伏夏纸药、夏尾回签、秋初回签、秋中馆脚、秋后馆账、秋尾炭脚与年关帖礼会不会在同一年里一旬旬咬回来。';
      pack.dossier = '举业结局=' + S.举业结局 + '｜生员=' + (S.生员身份 ? '是' : '否') + '｜优免=' + (S.优免启用 ? '启用' : '未启用') + '｜识字转业值=' + S.识字转业值;
