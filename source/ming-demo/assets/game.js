...
function applySeasonalElderFriction(log, stepLabel, season, xun, picked) {
  ...
  function stageFamily() {
    ...
    function familyRoutePack() {
      ...
      if (route.indexOf('路径一') === 0 || route.indexOf('留乡佃田') === 0) {
        ...
        pack.note = '学徒路成家后也不只是“在铺里站着”：铺里脚钱能不能捎回家、师门旧识还认不认你、哪一口人情能先替你打听差役，都会改写这一旬家计。现在连春头回铺回签、伏夏布药与回铺回签、秋里脚钱拆账与秋中回铺回签、年关铺账与冬尾回铺回签，也继续拆回同一年里逐旬结算。';
        pack.dossier = '学徒去向=' + S.学徒去向 + '｜授艺度=' + S.学徒授艺度 + '｜学徒历练=' + S.学徒历练;
        pack.event = { t: 'rel', tag: '[铺面]', txt: season.id === 'summer' && xun === 2
          ? '伏夏最怕的是人还站在铺里，家里却先缺了布药和针线；这一旬脚钱、布药和家口细耗会一起冒头。'
          : (season.id === 'autumn' && xun === 2
            ? '秋里脚钱看着比夏里厚些，可回铺回签、锅火、差钱和年关后手也正一齐来抢；不先拆账，就很容易误当“这一旬终于宽了”。'
            : (season.id === 'winter' && xun === 1
    ...
    return {
      ...
      settle: function (log) {
        ...
        if ((route.indexOf('路径三') === 0 || route.indexOf('入城学徒') === 0) && season.id === 'spring' && xun === 1) {
          if (picked.f_route_shop_spring_head_reply || picked.f_route_shop_note || picked.f_work || picked.f_repair || picked.f_rest) {
  ...
  function elderRoutePack() {
    ...
    if (isFarmRouteState() || isWageRouteState()) {
      ...
      pack.note = '举业一路到了晚年，看的是名色留下多少实际照应：生员能减一层外流，笔墨底子则更容易换来教馆、抄写和体面；更磨人的，是春头馆契、旧馆回话、伏夏馆汤、伏夏纸药、夏尾回签、秋初回签、秋中馆脚、秋尾回帖、秋尾炭脚、冬中馆札与年关帖礼会不会在同一年里一旬旬咬回来。';
      pack.dossier = '举业结局=' + S.举业结局 + '｜生员=' + (S.生员身份 ? '是' : '否') + '｜优免=' + (S.优免启用 ? '启用' : '未启用') + '｜识字转业值=' + S.识字转业值;
