  ...
  function stageFamily() {
    ...
    function familyRoutePack() {
      ...
      if (route.indexOf('路径一') === 0 || route.indexOf('留乡佃田') === 0) {
        ...
        pack.note = '卖工路成家后也不该只剩“这一年总共挣了多少工钱”。活路要先问、工食要分口回家、欠工要回头去结、差役也得先托旧工头探一层；现在又把春中回签与孩子草鞋、伏夏回签与锅火凉药、秋收旺工与秋中回签、冬头夹衣与回签门包、年关欠工、年下回签与来春工棚草鞋脚路继续拆回同一年里逐旬结算。';
    ...
    return {
      ...
      settle: function (log) {
        ...
        applySeasonalHouseholdFriction(log, stepLabel, season, xun, picked, {
          ...
          autumn: {
            handledIds: ['h_collect', 'h_pay', 'h_school_fund', 'h_clan', 'h_trust_field', 'h_side', 'h_wharf', 'h_autumn_tail', 'h_autumn_reply', 'h_autumn_receipt', 'h_autumn_sign', 'h_autumn_mid_reply'],
            doneTag: '秋后细账已拆',
