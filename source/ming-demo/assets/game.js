...
(function () {
  ...
  function noteExamIntraYearSignals(log, stepTag, beforeSignals) {
    ...
    if (support !== beforeSignals.support) {
      ...
      if (S.供读状态 === '已断供') {
        ...
        log.push(['〔供读口风〕这一旬过后，家里并未明着再续一口供读钱，你是靠誊抄、看账和手里临时拼出来的碎钱把这条读书路硬拖住。它既不算家中稳供，也还没彻底断线，只是缺口先由你自己顶在身上。', 'bad']);
      } else if (S.供读状态 === '观望供读') {
  ...
  function settleExamAdvanceCost(amount, opts) {
    amount = Math.max(0, Number(amount) || 0);
    opts = opts || {};
    ...
    noteExamOutlay(amount, {
      countSupport: opts.countSupport !== false,
      familySupport: opts.familySupport !== false
    });
    var parts = [];
  ...
  function applySeasonalExamFriction(log, stepLabel, season, xun, picked) {
    ...
    if (season.id === 'spring' && xun === 2) apply({
      handledIds: ['e_essay', 'e_mid_grain', 'e_mid_mother_help', 'e_home', 'e_rest', 'e_spring_packet'],
      doneTag: '春馆回话已理',
    ...
    if (season.id === 'summer' && xun === 2) apply({
      handledIds: ['e_essay', 'e_copy', 'e_mid_grain', 'e_mid_mother_help', 'e_mend', 'e_rest', 'e_summer_packet', 'e_summer_cough'],
      doneTag: '馆课零耗已顾',
    ...
    if (season.id === 'autumn' && xun === 2) apply({
      handledIds: ['e_guarantee', 'e_copy', 'e_mid_grain', 'e_mid_mother_help', 'e_home', 'e_autumn_packet'],
      doneTag: '秋后纸墨已拆',
    ...
    if (season.id === 'winter' && xun === 2) apply({
      handledIds: ['e_copy', 'e_mid_grain', 'e_mid_mother_help', 'e_mend', 'e_rest', 'e_winter_mid_packet', 'e_fail_copy', 'e_winter_cough'],
      doneTag: '冬中灯炭已分',
  ...
  function refreshExamSupportState() {
    ...
    var studyCount = (S.本年馆课次数 || 0) + (S.本年半读次数 || 0) + (S.本年寄读次数 || 0) + (S.本年投塾次数 || 0) + (S.本年评文次数 || 0) + (S.本年保结次数 || 0);
    var familySupportCount = (S.本年家中供读次 || 0) + (S.本年家中供读米 || 0);
    if (familySupportCount > 0) {
      S.供读状态 = '家中供读';
      return;
    }
    if (studyCount > 0 && ((S.本年誊抄次数 || 0) > 0 || (S.本年举业自筹文 || 0) >= 120)) {
      S.供读状态 = '自筹苦撑';
      return;
    }
    S.供读状态 = '观望供读';
...
function applySeasonalElderFriction(log, stepLabel, season, xun, picked) {
  ...
  function applySeasonalMerchantFriction(log, stepLabel, season, xun, picked) {
    ...
    if (season.id === 'autumn' && xun === 2) apply({
      handledIds: ['m_try', 'm_trial_capital', 'm_market', 'm_book', 'm_autumn_mid_bundle', 'm_autumn_mid_school', 'm_autumn_mid_drag'],
      doneTag: '试贩门包已分',
    ...
    if (season.id === 'winter' && xun === 1) apply({
      handledIds: ['m_collect', 'm_book', 'm_letter', 'm_reserve', 'm_mend', 'm_trial_capital', 'm_winter_head_packet', 'm_winter_head_school', 'm_corvee_split', 'm_winter_head_body'],
      doneTag: '年关路费已分',
      ...
      actions: function () {
        ...
        if ((season.id === 'autumn' && xun >= 1 && xun <= 2) || (season.id === 'winter' && xun === 1)) {
          A.push({
            ...
            cost: 1,
            eff: '铜钱-' + trialCapitalCost + '·家书+1·商信誉+1·议本+1',
            desc: season.id === 'winter'
              ? '冬清账上旬若还想试贩，就得先把“这一两试本从谁账上先挪、回话口风怎么走、来春本钱还留不留”说清。先把这层口风坐实，才不至像默认能直接动家里的银。'
              : (xun === 1
                ? '秋试手上旬若已开始探路，就得先把“这一两试本从谁账上先挪、牙行熟脸肯不肯替你递一句、家里认不认这口试手”坐实。先把这层议本口风拆开，秋中真试贩时才不至像默认能直接动家里的银。'
                : '秋试手中旬若真想带本试贩，就得先把“这一两试本从谁账上先挪、族叔肯不肯点头、回话门包由谁先出”坐实。先把这层议本口风拆开，试贩才不再像默认能直接动家里的银。'),
            can: S.本年商路议本 < 1 && S.铜钱 >= trialCapitalCost && (S.识货进度 >= 1 || S.账房进度 >= 1),
            why: S.本年商路议本 < 1
              ? (S.铜钱 >= trialCapitalCost
                ? ((S.识货进度 >= 1 || S.账房进度 >= 1)
                  ? ''
                  : '先学会一点认货或核账再谈试本')
                : ('铜钱不足' + trialCapitalCost + '文'))
              : '这一年已把试本口风坐实',
            once: true
          });
        }
        ...
        A.push({ id: 'm_try', name: isLate ? '赶在旬尾定试贩' : '争取带本试贩', cost: 2, eff: '白银-1锁作本钱·冬里按门路/账房/承继定位判回本/小利/亏折/未回款', desc: '拿一两本钱试着跑一单。前头得先把“谁肯让你动这一两、口风是否坐实”拆开记账；钱锁进货里后，回没回得来，才继续看你这一年把门路和账面坐实到哪一步。', can: ((season.id === 'autumn' && xun >= 2) || (season.id === 'winter' && xun === 1)) && S.本年商路试贩 < 1 && S.带本银 <= 0 && S.白银 >= 1 && (S.识货进度 >= 1 || S.账房进度 >= 1) && S.本年商路议本 > 0, why: ((season.id === 'autumn' && xun >= 2) || (season.id === 'winter' && xun === 1)) ? (S.本年商路试贩 < 1 ? (S.带本银 <= 0 ? (S.白银 >= 1 ? ((S.识货进度 >= 1 || S.账房进度 >= 1) ? (S.本年商路议本 > 0 ? '' : '还没先把试本口风坐实') : '尚未学会最基本认货/核账') : '白银不足1两') : '已有一笔本钱锁在货里') : '本年已试贩过一回') : '通常要到秋中旬以后才谈得上试贩', once: true });
        A.push({
          id: 'm_support',
          ...
          switch (p.id) {
            ...
            case 'm_trial_capital':
              if (spendCopper(trialCapitalCost)) {
                ...
                S.本年商路家书 += 1;
                S.商信誉 += 1;
                pushMerchantSeasonTag(season.name + xunLabel + '议试本');
                log.push([season.id === 'winter'
                  ? '先把试本回话与来春本钱分开：铜钱-55、家书+1、商信誉+1、议本+1。冬头先把“这一两试本从谁账上先挪、来春本钱还留不留”说清，试贩不再像默认能直接动家里的银。'
                  : (xun === 1
                    ? '先把试本口风与牙行照面坐实：铜钱-40、家书+1、商信誉+1、议本+1。秋头先把“这一两试本从谁账上先挪、熟脸肯不肯替你递一句”说清，秋中试贩不再像默认能直接动家里的银。'
                    : '先把试本口风与牙帖回话坐实：铜钱-45、家书+1、商信誉+1、议本+1。秋中先把“这一两试本从谁账上先挪、族叔肯不肯点头”说清，试贩不再像默认能直接动家里的银。'), 'good']);
              } else {
                log.push([season.id === 'winter'
                  ? '想先把试本回话与来春本钱分开，但这一旬铜钱已先紧，只得让试本与来春后手继续糊在一句“回头再说”里。'
                  : (xun === 1
                    ? '想先把试本口风与牙行照面坐实，但这一旬铜钱已先紧，只得让这一两试本继续停在一句“回头再探”里。'
                    : '想先把试本口风与牙帖回话坐实，但这一旬铜钱已先紧，只得让这一两试本继续停在一句“回头再议”里。'), 'bad']);
              }
              break;
        ...
        if (xun === 1) {
          if ((S.投塾进度 || 0) < 2 && S.供读状态 !== '已断供') {
            A.push({
              ...
              cost: 1,
              eff: '尝试推进投塾进度·若塾门未点头则只落帖样脚费',
              desc: season.id === 'spring'
                ? '先把塾师肯不肯收、帖子怎么递、第一口束脩从谁账上先压进去坐实。钱得真花，塾门却未必这一旬就肯点头。'
                : '塾门不续、回帖不来，后头的馆课、评文和应试都容易变成空话。这一旬就算花了帖样脚费，也可能只换来一句“再看看”。',
              can: S.供读状态 !== '已断供',
              once: true
            });
          }
          ...
          A.push({
            ...
            eff: '存米-1·铜钱+' + grainSupportCopper + '·供读压力-1',
            desc: season.id === 'spring'
              ? '春课下旬若不再明着从口粮里拆一石出来，香纸、回馆脚费与春尾锅火的尾账还是会继续追着这条供读线。'
              : (season.id === 'summer'
                ? '伏夏下旬若不再从存米里拆一石出来，衣药、灯油与潮纸尾账就会继续贴着身子和家计一起咬钱。'
                : (season.id === 'autumn'
                  ? '秋试下旬若不再明着粜一石口粮，簿册纸样、临场脚费与秋尾锅火还是会继续压在同一口现钱上。'
                  : '冬清账下旬若不再明着挪一石存米，门包、灯炭与来春投帖后手就会继续贴着年关锅火一起追钱。')),
            can: S.存米 >= 1 && S.供读状态 !== '已断供',
            why: S.存米 >= 1 ? (S.供读状态 !== '已断供' ? '' : '家中已断供') : '存米不足1石',
            once: true
          });
          A.push({
            id: 'e_tail_mother_help',
            name: season.id === 'spring'
              ? '托母亲纺线钱垫春尾香纸'
              : (season.id === 'summer'
                ? '托母亲纺线钱续夏尾衣药'
            ...
            eff: '铜钱+' + motherHelpCopper + '·供读压力-1·家族+1',
            desc: season.id === 'spring'
              ? '春课下旬若母亲肯再从纺织私账里匀出一口钱，香纸、回馆脚费与春尾锅火就不必继续都压在父兄那边。'
              : (season.id === 'summer'
                ? '伏夏下旬若母亲肯再匀一口纺线钱，衣药、灯油与潮纸尾账就不必继续只靠家里口粮去换。'
                : (season.id === 'autumn'
                  ? '秋试下旬若母亲肯再卖一口布，簿册纸样和秋尾锅火就不必继续全挤在父兄那口现钱上。'
                  : '冬清账下旬若母亲肯再匀一口纺线钱，门包、灯炭与旧馆回话尾账就不必全压在年关锅火上。')) + ' 这口钱仍来自母亲自愿拆出的私账，不是户主公账自动拨下。',
            can: S.供读状态 !== '已断供' && (S.本年母纺贴补次 || 0) < 3,
            why: S.供读状态 !== '已断供' ? ((S.本年母纺贴补次 || 0) < 3 ? '' : '本年母纺贴补已到3回') : '家中已断供',
            once: true
          });
          if (season.id === 'spring') {
        ...
        lifePicks.forEach(function (p) {
          ...
          switch (p.id) {
            case 'e_enroll':
              ...
              S.本年投塾次数 += 1;
              S.本年延婚牵扯 += 1;
              var enrollGateOk = examEnrollGateScore() >= examEnrollGateTarget();
              if (enrollGateOk) {
                ...
                S.家族 = Math.max(0, S.家族 - 1);
                S.供读压力 += 1;
                S.本年纸墨支出文 += Math.min(15, enrollCost);
                S.本年零耗支出文 += Math.max(0, enrollCost - 15);
                pushExamSeasonTag(stepTag + '塾门回话未定');
                log.push([
                  (season.id === 'spring' ? '先递塾帖试坐馆' : (season.id === 'summer' ? '催塾门回话' : '补一道塾门回帖'))
                    + '：这一旬只把帖样、人情与脚费先递到了'
                    + enrollPay.text
                    + '，塾门却还只肯说“再看看”；投塾进度没坐实、家族-1、供读压力+1。钱已经花出去，但这口回话并不会自动给你。',
                  'bad'
                ]);
              }
              break;
            ...
            case 'e_copy':
              ...
              var copyPaperPay = settleExamAdvanceCost(copyPaperCost, { countSupport: false, familySupport: false });
            ...
            case 'e_tail_grain':
              if (S.存米 >= 1) {
                ...
                S.本年家中供读米 += 1;
                S.本年延婚牵扯 += 1;
                pushExamSeasonTag(stepTag + (p.id === 'e_mid_grain' ? '米脚续供' : (p.id === 'e_tail_grain' ? '米脚收尾' : '米脚供读')));
                log.push([
                  p.name
