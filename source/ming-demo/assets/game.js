...
(function () {
  ...
  function initState(carry, opts) {
    ...
    S = {
      ...
      本年束脩支出文: 0, 本年纸墨支出文: 0, 本年保结支出文: 0, 本年盘缠支出文: 0, 本年已落举业支出文: 0, 本年家中供读次: 0, 本年家中供读文: 0, 本年家中供读米: 0, 本年家中贴补次: 0, 本年家中贴补米: 0, 本年落第次数: 0, 本年身子亏空: 0, 本年延婚牵扯: 0,
      // 人生链路字段
  ...
  function applySeasonalExamFriction(log, stepLabel, season, xun, picked) {
    ...
    if (season.id === 'spring' && xun === 1) apply({
      handledIds: ['e_enroll', 'e_tutor', 'e_school', 'e_half', 'e_family_grain', 'e_spring_open_packet'],
      doneTag: '春课开销已理',
    ...
    if (season.id === 'summer' && xun === 1) apply({
      handledIds: ['e_enroll', 'e_tutor', 'e_school', 'e_home', 'e_family_grain', 'e_summer_open_packet'],
      doneTag: '伏夏馆账已顾',
    ...
    if (season.id === 'autumn' && xun === 1) apply({
      handledIds: ['e_enroll', 'e_tutor', 'e_home', 'e_rest', 'e_family_grain', 'e_autumn_open_packet'],
      doneTag: '秋前盘缠已理',
    ...
    if (season.id === 'autumn' && xun === 3) apply({
      handledIds: ['e_exam', 'e_reserve', 'e_mend', 'e_home', 'e_autumn_tail_packet'],
      doneTag: '临场盘缠已留',
    ...
    if (season.id === 'winter' && xun === 1) apply({
      handledIds: ['e_home', 'e_rest', 'e_copy', 'e_mend', 'e_winter_open_packet'],
      doneTag: '年关纸墨已分',
...
function applySeasonalElderFriction(log, stepLabel, season, xun, picked) {
  ...
  function applySeasonalMerchantFriction(log, stepLabel, season, xun, picked) {
    ...
    if (season.id === 'winter' && xun === 2) apply({
      handledIds: ['m_collect', 'm_book', 'm_letter', 'm_reserve', 'm_clear_packet', 'm_debt_split', 'm_winter_family_split'],
      doneTag: '清账回话已压',
  ...
  function stageCivilExam() {
    ...
    return {
      ...
      note: '举业路现改成“春课→夏课→秋试→冬清账”四季、每季三旬：上旬先定主读法、投塾与供读口风，中旬再磨文章、跑资格、补识字、接誊抄，下旬把应场、回家缓家计、差役钱与衣药后手一笔笔收紧。每旬默认按“三手并行”来想，一手顾课业主线，一手拆家计或门路碎账，再留一手给身子、差役或回乡后手；若家里真要续供，也得显式粜米或另挪口粮来换纸墨盘缠，而不是把存米自动折成现银。供读不推出录取，仍不展开乡试会试。',
      ...
      dossier: function () {
        return lifeDossier('当前举程=' + season.name + '·' + xunLabel + '｜投塾=' + examEnrollmentLabel(S.投塾进度) + '｜童试层级=' + examTierLabel(S.童试层级, S.生员身份) + '｜保结=' + examGuaranteeLabel(S.保结进度) + '｜文章火候=' + S.文章火候 + '｜供读状态=' + S.供读状态 + '｜本年投塾=' + S.本年投塾次数 + '｜识字旬=' + S.本年识字旬数 + '｜馆课=' + S.本年馆课次数 + '｜半读=' + S.本年半读次数 + '｜评文=' + S.本年评文次数 + '｜保结奔走=' + S.本年保结次数 + '｜誊抄=' + S.本年誊抄次数 + '｜归家缓家=' + S.本年归家次数 + '回/' + S.本年家中贴补米 + '石｜家中供读=' + S.本年家中供读次 + '回/' + S.本年家中供读文 + '文/' + S.本年家中供读米 + '石｜已落举业支出=' + S.本年已落举业支出文 + '文｜束脩=' + S.本年束脩支出文 + '文｜纸墨=' + S.本年纸墨支出文 + '文｜保结脚费=' + S.本年保结支出文 + '文｜盘缠=' + S.本年盘缠支出文 + '文｜落第=' + S.本年落第次数 + '｜延婚牵扯=' + S.本年延婚牵扯 + '｜身子亏空=' + S.本年身子亏空 + (S.生员身份 ? '｜已是生员' : '') + '。');
      },
      events: [
        {
          t: 'rel',
      ...
      settle: function (log) {
        ...
        lifePicks.forEach(function (p) {
          ...
          switch (p.id) {
            ...
            case 'e_school':
              ...
              S.文章火候 += 1; S.读书成本档 += 1; S.读书方式 = '社学寄读'; S.本年寄读次数 += 1; S.本年束脩支出文 += 60; S.本年纸墨支出文 += 20; S.本年延婚牵扯 += 1; S.投塾进度 = Math.max(1, S.投塾进度 || 0); if (S.保结进度 < 1) S.保结进度 = 1; didStudy = true;
            ...
            case 'e_guarantee':
              ...
              S.保结进度 = Math.min(2, Math.max(1, S.保结进度) + 1); S.本年保结次数 += 1; S.本年保结支出文 += 80; S.本年延婚牵扯 += 1;
            ...
            case 'e_family_grain':
              if (S.存米 >= 1) {
                ...
                S.本年家中供读米 += 1;
                S.本年延婚牵扯 += 1;
                pushExamSeasonTag(stepTag + '米脚供读');
                log.push([
                  (season.id === 'autumn'
                    ? '先粜一石口粮凑盘缠'
                    : (season.id === 'winter'
        ...
        if (studyCost > 0) {
          ...
          if (S.铜钱 >= studyCost) {
            S.铜钱 -= studyCost;
            log.push(['〔束脩纸墨〕前面逐旬已落了 ' + S.本年已落举业支出文 + ' 文，本年余下总账再收尾：铜钱-' + studyCost + '（全年约束脩' + S.本年束脩支出文 + '、纸墨' + S.本年纸墨支出文 + '、保结脚费' + S.本年保结支出文 + '、盘缠' + S.本年盘缠支出文 + '）。', 'bad']);
          } else {
            ...
            if (S.白银 >= silverNeed) {
              S.白银 -= silverNeed;
              log.push(['〔束脩纸墨〕前面逐旬已落了 ' + S.本年已落举业支出文 + ' 文，年终余账仍要再从家里硬银上支 ' + silverNeed + ' 两；里头还带着今年投塾、纸墨和盘缠的后手。', 'bad']);
            } else {
          ...
          log.push(['〔束脩纸墨〕这一年束脩、纸墨、保结与盘缠大头已在春夏秋冬逐旬落账，年终只剩零头，不再整笔挤到冬尾。', 'good']);
        }
        if (studyCost > 0) {
          var familySupportNeed = Math.max(0, S.本年家中供读文 - (S.本年誊抄次数 * copyCopper));
          if (familySupportNeed > 0 || S.本年归家次数 > 0 || S.本年家中贴补米 > 0 || S.本年家中供读米 > 0) {
            log.push(['〔家中供养〕这一年家里先往你这边压了约 ' + S.本年家中供读文 + ' 文的供读钱，里头含投塾、束脩、纸墨与盘缠；另有 ' + S.本年家中供读米 + ' 石存米被明着粜作纸墨盘缠。你也回家缓了 ' + S.本年归家次数 + ' 旬、帮家贴回 ' + S.本年家中贴补米 + ' 石。供读只是让钱和口粮继续往你这边压，不推出录取，也不把米自动折银。', 'bad']);
          }
        } else if (S.本年家中供读文 > 0 || S.本年家中供读米 > 0) {
          log.push(['〔家中供养〕这一年供读钱多半已在逐旬直接落账：家里先往你这边压了约 ' + S.本年家中供读文 + ' 文' + (S.本年家中供读米 > 0 ? ('，并另粜了 ' + S.本年家中供读米 + ' 石存米换纸墨脚费') : '') + '；投塾、束脩、纸墨与盘缠都已经在旬内现形。供读仍只代表家计让渡，不代表录取。', 'bad']);
        }
