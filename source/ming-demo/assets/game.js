...
(function () {
  ...
  function applySeasonalExamFriction(log, stepLabel, season, xun, picked) {
    ...
    if (season.id === 'spring' && xun === 1) apply({
      handledIds: ['e_tutor', 'e_school', 'e_half'],
      doneTag: '春课开销已理',
    ...
    if (season.id === 'spring' && xun === 2) apply({
      handledIds: ['e_essay', 'e_home', 'e_rest'],
      doneTag: '春馆回话已理',
    ...
    if (season.id === 'spring' && xun === 3) apply({
      handledIds: ['e_copy', 'e_home', 'e_rest'],
      doneTag: '春尾香纸已分',
    ...
    if (season.id === 'summer' && xun === 1) apply({
      handledIds: ['e_tutor', 'e_school', 'e_home'],
      doneTag: '伏夏馆账已顾',
    ...
    if (season.id === 'summer' && xun === 3) apply({
      handledIds: ['e_copy', 'e_mend', 'e_rest', 'e_home'],
      doneTag: '夏尾衣药已分',
    ...
    if (season.id === 'autumn' && xun === 1) apply({
      handledIds: ['e_tutor', 'e_home', 'e_rest'],
      doneTag: '秋前盘缠已理',
    ...
    if (season.id === 'autumn' && xun === 3) apply({
      handledIds: ['e_exam', 'e_reserve', 'e_mend', 'e_home'],
      doneTag: '临场盘缠已留',
    ...
    if (season.id === 'winter' && xun === 2) apply({
      handledIds: ['e_copy', 'e_mend', 'e_rest'],
      doneTag: '冬中灯炭已分',
    ...
    if (season.id === 'winter' && xun === 3) apply({
      handledIds: ['e_reserve', 'e_home', 'e_mend', 'e_rest', 'e_exam'],
      doneTag: '冬尾门包已留',
...
function applySeasonalElderFriction(log, stepLabel, season, xun, picked) {
  ...
  if (isExamElder && season.id === 'autumn' && xun === 3) apply({
    handledIds: ['e_tutor_autumn_bundle_old', 'e_rest'],
    doneTag: '秋尾馆炭已分',
    doneLog: '〔秋尾馆炭〕这一旬先把炭脚、锅火零用和学生家小回礼分开了；举业路晚景最怕“秋账刚结回，冬里帖费又要起”的那层尾账，没有再被顺手拖进年关。',
    cost: 35,
    costTag: '秋尾馆炭',
    costLog: '〔秋尾馆炭〕炭脚、锅火零用和学生家小回礼一起要钱：铜钱-{cost}。不是大账，却正把举业路老年秋尾最细、也最容易被忽略的炭脚账重新压回这一旬。',
    failTag: '秋尾馆炭硬顶',
    failLog: '〔秋尾馆炭〕这一旬连炭脚和锅火零用都腾挪不开，只得先硬顶过去；秋尾这层身子与门路的后手又薄了一线（体魄-1）。',
    hardship: 'body'
  });
    ...
    return {
      ...
      settle: function (log) {
        ...
        if ((route.indexOf('路径二') === 0 || route.indexOf('受雇') === 0) && season.id === 'spring' && xun === 2) {
          if (picked.f_route_wage_spring_bundle || picked.f_route_send || picked.f_child || picked.f_kitchen) {
            pushFamilySeasonTag(stepTag + '春锅草鞋已分');
            log.push(['〔春锅草鞋〕这一旬先把锅火、草鞋和家里那层零碎小耗拆开了；春里刚回手的工钱没有再一转身就漏成空话。', 'good']);
          } else if (spendCopper(35)) {
        ...
        if ((route.indexOf('路径二') === 0 || route.indexOf('受雇') === 0) && season.id === 'summer' && xun === 2) {
          if (picked.f_route_wage_summer_bundle || picked.f_route_send || picked.f_market || picked.f_child || picked.f_kitchen) {
        ...
        if ((route.indexOf('路径二') === 0 || route.indexOf('受雇') === 0) && season.id === 'autumn' && xun === 2) {
          if (picked.f_route_wage_autumn_split || picked.f_social || picked.f_market || picked.f_child || picked.f_route_send) {
            pushFamilySeasonTag(stepTag + '秋工锅火已分');
            log.push(['〔秋工锅火〕这一旬先把旺工茶水、回乡脚费、锅火小耗和差钱后手分开了；秋工钱看着厚一点，也没有再被你误写成“这一旬自然宽了”。', 'good']);
          } else if (spendCopper(50)) {
        ...
        if ((route.indexOf('路径二') === 0 || route.indexOf('受雇') === 0) && season.id === 'winter' && xun === 3) {
          if (picked.f_route_wage_winter_post || picked.f_route_wage_collect || picked.f_route_wage_duty || picked.f_duty) {
            pushFamilySeasonTag(stepTag + '明春工路已问');
            log.push(['〔来春工路〕这一旬先把明春头程脚费、回话脚路和旧工头那层熟面留住了；卖工路没有在冬尾把来年第一程重新拖回冷面求人。', 'good']);
          } else if (spendCopper(40)) {
            pushFamilySeasonTag(stepTag + '来春工路');
            log.push(['〔来春工路〕明春头程脚费、回话脚路和旧工头递话小钱一起要钱：铜钱-40。不是大账，却正把“明春第一口活从哪来”逼回这一旬。', 'bad']);
          } else {
            S.家族 = Math.max(0, S.家族 - 1);
            pushFamilySeasonTag(stepTag + '来春工路硬顶');
            log.push(['〔来春工路〕这一旬连明春头程脚费都腾挪不开，只得先硬顶过去；旧工头和明春工路这层熟面又薄了一线（家族-1）。', 'bad']);
          }
        }
        ...
        if ((route.indexOf('路径四') === 0 || route.indexOf('徽商') === 0) && season.id === 'summer' && xun === 3) {
          if (picked.f_route_collect || picked.f_repay || picked.f_mend || picked.f_route_sample || picked.f_route_letter || picked.f_route_summer_reply) {
        ...
        if ((route.indexOf('路径四') === 0 || route.indexOf('徽商') === 0) && season.id === 'autumn' && xun === 1) {
          if (picked.f_route_autumn_quote || picked.f_route_autumn_packet || picked.f_work || picked.f_market || picked.f_child || picked.f_route_letter) {
        ...
        if ((route.indexOf('路径四') === 0 || route.indexOf('徽商') === 0) && season.id === 'autumn' && xun === 3) {
          if (picked.f_route_receipt || picked.f_route_collect || picked.f_route_autumn_quote || picked.f_route_autumn_tail || picked.f_social || picked.f_duty) {
        ...
        if ((route.indexOf('路径四') === 0 || route.indexOf('徽商') === 0) && season.id === 'autumn' && xun === 3) {
          if (picked.f_route_autumn_tail || picked.f_route_receipt || picked.f_route_school || picked.f_route_collect || picked.f_duty) {
        ...
        if ((route.indexOf('路径三') === 0 || route.indexOf('入城学徒') === 0) && season.id === 'summer' && xun === 1) {
          if (picked.f_route_shop_note || picked.f_work || picked.f_repair || picked.f_child || picked.f_mend) {
        ...
        if ((route.indexOf('读书应举') === 0 || S.举业结局 !== '未定' || S.生员身份) && season.id === 'autumn' && xun === 2) {
          if (picked.f_route_school_split || picked.f_route_school_autumn_reply || picked.f_social || picked.f_market || picked.f_route_write) {
        ...
        if (isYearEnd) {
          ...
          if ((route.indexOf('路径二') === 0 || route.indexOf('受雇') === 0) && (S.本年家季务 || []).some(function (tag) { return String(tag).indexOf('秋工锅火') >= 0 || String(tag).indexOf('秋尾差脚') >= 0; })) log.push(['这一养家年你还把秋工锅火、差钱后手和秋尾回话门包一起摊回了秋中秋尾；旺工钱终于不再只是“挣得多”，而是当旬就要被家用、制度和人情拆开。', 'good']);
    ...
    return {
      ...
      settle: function (log) {
        ...
        applySeasonalHouseholdFriction(log, stepLabel, season, xun, picked, {
          summer: {
            handledIds: ['h_hire', 'h_side', 'h_rest', 'h_proxy_wage', 'h_hold_field', 'h_literate', 'h_wage_summer_note', 'h_wage_summer_split'],
            doneTag: '伏夏小耗已顾',
          ...
          autumn: {
            handledIds: ['h_wage_collect', 'h_proxy_wage', 'h_pay', 'h_lease_home', 'h_clan', 'h_side', 'h_wage_autumn_note', 'h_wage_autumn_tail'],
            doneTag: '秋后细账已拆',
        ...
        if (season.id === 'summer' && xun === 2) {
          if (picked.h_hold_field || picked.h_hire || picked.h_side || picked.h_rest || picked.h_literate || picked.h_wage_summer_split) {
      ...
      settle: function (log) {
        ...
        if (season.id === 'autumn' && xun === 3) {
          if (picked.h_autumn_tail || picked.h_shop_collect || picked.h_proxy || picked.h_side || picked.h_clan) {
            ...
            log.push(['〔秋尾回话〕铺里回话脚费、催佃脚路和锅火小耗一起要钱：铜钱-45。不是新主线，却把学徒路当户秋尾那层“钱说快回、碎账先到”的摩擦重新压回这一旬。', 'bad']);
          } else {
            S.家族 = Math.max(0, S.家族 - 1);
            pushHouseholdSeasonTag(stepLabel + '秋尾硬顶');
            log.push(['〔秋尾回话〕这一旬连回话脚费和催佃脚路都腾挪不开，只得先硬顶过去；铺里与乡里两头替这一房接气的门路又薄了一线（家族-1）。', 'bad']);
          }
        }
        if (season.id === 'winter' && xun === 1) {
  ...
  function stageMerchantHousehold() {
    ...
    return {
      ...
      settle: function (log) {
        ...
        applySeasonalHouseholdFriction(log, stepLabel, season, xun, picked, {
          summer: {
            handledIds: ['h_collect', 'h_school_fund', 'h_side', 'h_rest', 'h_literate', 'h_clan', 'h_wharf', 'h_summer_tail', 'h_summer_cool', 'h_summer_market'],
            doneTag: '伏夏小耗已顾',
          ...
          autumn: {
            handledIds: ['h_collect', 'h_pay', 'h_school_fund', 'h_clan', 'h_trust_field', 'h_side', 'h_wharf', 'h_autumn_tail', 'h_autumn_receipt'],
            doneTag: '秋后细账已拆',
          ...
          winter: {
            handledIds: ['h_pay', 'h_collect', 'h_literate', 'h_school_fund', 'h_clan', 'h_side', 'h_rest', 'h_wharf', 'h_winter_route_split', 'h_winter_gift'],
            doneTag: '年关碎账已分',
        ...
        if (season.id === 'summer' && xun === 1) {
          if (picked.h_summer_cool || picked.h_wharf || picked.h_side || picked.h_rest || picked.h_collect) {
        ...
        if (season.id === 'spring' && xun === 2) {
          if (picked.h_collect || picked.h_trust_field || picked.h_literate || picked.h_clan || picked.h_wharf || picked.h_spring_deed) {
        ...
        if (season.id === 'winter' && xun === 1) {
          if (picked.h_winter_gift || picked.h_wharf || picked.h_side || picked.h_collect || picked.h_pay || picked.h_rest) {
        ...
        if (season.id === 'winter' && xun === 3) {
          if (picked.h_winter_tail || picked.h_winter_coal || picked.h_school_fund || picked.h_wharf) {
  ...
  function stageExamHousehold() {
    ...
    return {
      ...
      settle: function (log) {
        ...
        applySeasonalHouseholdFriction(log, stepLabel, season, xun, picked, {
          spring: {
            handledIds: ['h_copy_mid', 'h_exempt', 'h_literate', 'h_clan', 'h_spring_packet'],
            doneTag: '分书碎费已理',
          ...
          autumn: {
            handledIds: ['h_pay', 'h_copy_mid', 'h_clan', 'h_exam_lease', 'h_side', 'h_literate', 'h_exam_split', 'h_autumn_packet'],
            doneTag: '秋后细账已拆',
          ...
          winter: {
            handledIds: ['h_exempt', 'h_copy_mid', 'h_pay', 'h_literate', 'h_side', 'h_rest', 'h_exam_split', 'h_winter_packet'],
            doneTag: '年关碎账已分',
          ...
          winterLower: {
            handledIds: ['h_exam_split', 'h_copy_mid', 'h_winter_tail', 'h_rest'],
            doneTag: '冬尾笔炭已理',
  ...
  function elderRoutePack() {
    ...
    if (isFarmRouteState() || isWageRouteState()) {
      ...
      pack.note = '学徒一路到了晚年，看的是城中门路有没有坐实：留店、坐店工、跟货，都会改变你老来靠谁照应。养老不只剩“诸子养不养”，还要看旧掌柜肯不肯回话、伏夏铺药与带话脚费能不能先顾住、铺里旧脚钱与分得薄田的租谷回不回得来、来春回铺脚路有没有在今冬先留出来。';
      ...
      pack.note = '举业一路到了晚年，看的是名色留下多少实际照应：生员能减一层外流，笔墨底子则更容易换来教馆、抄写和体面；更磨人的，是旧馆回话、伏夏纸药、秋初回签、秋后馆账、秋尾炭脚与年关帖礼会不会在同一年里一旬旬咬回来。';
      pack.dossier = '举业结局=' + S.举业结局 + '｜生员=' + (S.生员身份 ? '是' : '否') + '｜优免=' + (S.优免启用 ? '启用' : '未启用') + '｜识字转业值=' + S.识字转业值;
