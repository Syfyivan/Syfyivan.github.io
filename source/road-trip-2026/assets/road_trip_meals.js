(function () {
  ...
  // 写清这顿从哪来（常温速食/干粮）、怎么加热、断电或吃不上怎么办；不写成空话。
  // 补货的关键城市节点（敦煌D6、喀什D20/22、拉萨D33、丽江D55、成都D63）单独提示。
  const RESTOCK_NOTE = {
    6: '敦煌住整套公寓：当天在菜市买菜，晚上厨房现做现吃、当天光盘（不冻不跨夜）；进疆前把自热饭/米线/料包和耐放蔬果补满。',
    20: '喀什是新藏线前最后大补给：异丁烷/丙烷高山混合气（勿买纯丁烷）、自热饭、自热麻辣烫、米线泡面、多样料包、干粮和耐放蔬果+鸡蛋补满；饮用水按每人每天3L×4天≈48L（5L桶装8–10桶）备满，备足到能撑过阿里段。',
    22: '喀什补给窗口：上阿里前把速食和干粮备足到能撑过狮泉河，之后 3–4 天几乎无补给，全程不依赖冷冻。',
    33: '拉萨休整窗口长：进店认真吃几顿有蔬菜蛋白的正餐，速食库存补到给滇藏线和川西够用。',
    55: '丽江是返程成熟城市：按需补速食和气罐，蔬果随买随吃，不用大囤；住整套公寓可当天买菜现做一顿。',
    63: '成都补给：给 D65 成都→西安长途高速日备好自热饭/米线和干粮；住公寓当天现做一顿。'
  };
  const carryNote = day => {
  ...
  window.ROAD_TRIP_MEAL_DATA = {
    meta: {
      ...
      kitchenNights: [],
      plannedCookDinners: [],
      note: '不设固定自炊日。原则：吃饭方便就进店尝当地特色；只有偏远、吃不惯或吃饭不便的节点才考虑自己弄（民宿开火或常温速食均可，并遵守当地防火管制）。是否自炊按当天便利度临时决定，不写死在计划里。长途午餐默认常温速食（自热米饭·自热麻辣烫、米线泡面或户外煮），优先路过可靠县城热食；非必要不在高速服务区吃饭，实在赶不上才在服务区吃几顿。'
    },
    dinnerCounts,
    meals
  };
