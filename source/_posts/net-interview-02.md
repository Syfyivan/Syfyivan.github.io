...
<div class="niv-a">
<strong>一句话串讲版（背这个）：</strong>浏览器先解析 URL 拿到协议、域名、端口、路径；接着做代理决策，判断这个请求是直连还是走代理；然后 DNS 把域名解析成 IP（走代理时可能由代理来解析）；接着和目标（或代理）做 TCP 三次握手；如果是 HTTPS 再做一次 TLS 握手协商密钥；然后发 HTTP 请求；服务器返回响应；浏览器解析 HTML 并继续加载子资源、渲染页面；最后连接被复用或关闭。
</div>
