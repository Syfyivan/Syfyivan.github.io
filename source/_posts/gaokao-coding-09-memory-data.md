...
思路：你填的内容，要"存"到某个地方，刷新还在。比如给主页加个小记事、或自己的留言草稿，关掉再打开还在。

最简单的版本，是存在**浏览器本地**（叫 localStorage，可以理解成"这个网页在你这台设备上的小抽屉"）。

<div class="gkc-note"><strong>先说清一个重要的"但是"：</strong>本地保存只存在<strong>你自己这台设备</strong>上。所以它做"给自己看的小记事"很合适；但如果你想要的是"<strong>开学发给同学、大家来给你留言、你能收到</strong>"——那 localStorage 做不到（别人留的言只存在他自己手机里，你看不到）。真要那种，需要把数据存到<strong>后端 / 数据库</strong>，那是更大的话题，等你将来学到数据库那门课就明白了，这里先不深入。</div>
...
第三站收工！你的作品现在能对话、有灵魂、（也许）还有记忆。第 07 讲接 AI 时，你可能已经顺手拿到了一个能打开的网址——但"上线 / 部署"这件事我们还没正式讲透，纯网页的作品可能也还没上线。最后一站，我们就把这件最爽的事做扎实——**把作品正式送上线、变成一个谁都能放心点开的网址**，开学就能甩进新生群。下一讲 **第 10 讲**，见证高光时刻。
