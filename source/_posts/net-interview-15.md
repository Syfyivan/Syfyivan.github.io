...
### 先想清楚：三次握手只建了「到代理的管子」，目标信息还没给

很多人一上来就懵在这里：既然三次握手是和**代理**握的，那代理怎么知道我真正想访问哪个网站？关键要意识到——**TCP 三次握手里只有 IP 和端口，不含任何「我要去哪个 URL」的信息**，它只负责把你和代理之间的这根「管子」建起来。所以握完手之后，**必然还有一步专门把目标告诉代理**，代理才知道往哪转发。这一步长什么样，取决于代理类型和你访问的是 HTTP 还是 HTTPS：

**情况一：走 HTTP 代理、访问明文 HTTP。** 直连时请求行只写路径（`GET /page HTTP/1.1`）；但走代理时，浏览器把**完整 URL 直接写进请求行**发给代理：

```text
① 客户端 ──► 代理：  跟代理三次握手（建到代理的 TCP）
② 客户端 ──► 代理：  GET http://example.com/page HTTP/1.1   ← 是完整 URL，不是 /page
                    Host: example.com
③ 代理 ──► 目标：    代理据此自己去 DNS、连 example.com、把请求转过去
④ 代理 ──► 客户端：  把目标的响应原样带回来
```

代理就是靠第 ② 步请求行里的**绝对 URL**知道目标是谁的。因为是明文，代理能读懂内容，也能顺手做缓存、过滤。

### HTTP 代理怎么转发 HTTPS：CONNECT 隧道

**情况二：走 HTTP 代理、访问 HTTPS。** 这是最容易被追问、也最能体现理解深度的一块。HTTPS 是加密的，浏览器没法把请求明文塞给代理让它读，那怎么告诉代理目标、又怎么转发？答案是 `CONNECT` 隧道——**先用一条明文的 CONNECT 报出目标建好隧道，之后所有加密数据都从隧道里穿过去**：
