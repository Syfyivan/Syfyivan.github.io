...
**为什么需要它**：理想情况下，靠「会话 id 出现在进程命令行里」就能找到进程。但有些工具（比如 Claude Code）**不把 session id 放在命令行参数上**——它的会话是按文件存的。这时候 `ps` 里搜不到 id，就找不到进程、也就拿不到 tty。

兜底办法：换一个「连接键」——**cwd**。会话信息里一般带着 cwd。于是：枚举所有 `claude`/`codex` 进程，逐个查它们的 cwd，和会话的 cwd 比对，匹配上的就是目标进程，再拿它的 tty。
