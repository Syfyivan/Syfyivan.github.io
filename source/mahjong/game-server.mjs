const nodeRuntime = typeof process !== "undefined" && Boolean(process.versions && process.versions.node);
let createHash;
let randomUUID;
...
if (nodeRuntime) {
  ...
  networkInterfaces = osModule.networkInterfaces;
  ({ dirname, extname, join, resolve, sep } = pathModule);
...
const variants = {
  sichuan: {
    ...
    rules: [
      "108 张数牌，无风箭字牌",
      "不能吃牌，可碰、杠、胡",
      "计分：点炮 2，自摸每家 2；清一色、七对、碰碰胡和杠上开花另加分",
      "杠分即时记账，结算时统一计入总分"
    ],
    detailedRules: [
      {
      ...
      {
        title: "胡牌",
        items: ["本版川麻先使用基础胡牌判定：4 组面子 + 1 对将，或七对子。", "点炮胡由点炮者付 2 分；自摸每家付 2 分。"]
      },
      {
        title: "本版计分",
        items: ["清一色加 4 分，七对加 4 分，碰碰胡加 2 分，杠上开花加 2 分。", "点杠由点杠者付 1 分；补杠每家付 1 分；暗杠每家付 2 分。", "杠分先记账，本局胡牌或流局后统一结算到总分。"]
      },
      {
        title: "杠分",
        items: ["碰出的刻子补成四张按补杠；手里四张暗刻亮出按暗杠；别人打出第四张按点杠。", "杠后立即补摸一张，本版已支持杠上开花加分。"]
      }
    ]
  },
  dongbei: {
    ...
    rules: [
      ...
      "计分：普通胡 2；宝胡每家 8；七对、清混一色、漂胡、杠上开花和杠分另计"
    ],
    detailedRules: [
      {
        title: "牌组和人数",
      ...
      },
      {
        ...
        items: [
          ...
          "对宝胡 8、摸宝胡 8：由其余每家支付；七对加 2 分，清一色加 4 分，混一色加 2 分，漂胡/碰碰胡加 2 分，杠上开花加 2 分。",
...
export function startServer(preferredPort = defaultPort, attempts = 0) {
  if (!nodeRuntime) {
    throw new Error("HTTP server is only available in Node.js");
  }
