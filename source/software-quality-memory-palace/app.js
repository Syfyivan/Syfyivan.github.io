const rooms = [
  {
    ...
    floor: "大门",
    label: "质量大门",
    short: "客户 账本 公章 尺子 预报",
    icon: "质",
    color: "#326aa3",
    scene: "门口站着客户，手里拿需求单；旁边有账本、公章、尺子和天气预报。五件道具比五个字头更好想。",
    mnemonic: "质量门口五件道具：客户、账本、公章、尺子、天气预报。",
    points: ["客户属性：客户拿需求单", "成本属性：账本算代价", "社会属性：公章代表社会责任", "可测性：尺子能量出来", "可预见性：天气预报能提前判断", "质量由客户判定，既满足明示需求，也满足隐含需求"],
  ...
  },
  {
    ...
    icon: "保",
    color: "#22776d",
    scene: "一楼有三个窗口：总部部门窗口、项目工程师窗口、小组窗口。前台桌上排着计划表、过程图、评审章、审计灯、记录本、报告麦、跟踪绳、度量尺。",
    mnemonic: "SQA 先看三窗口，再看八件办公道具。",
    ...
    prompt: "看到三个窗口，先说组织结构；看到八件办公道具，再说计划、过程、评审、审计、记录、报告、跟踪、度量。"
  },
  {
    id: "review",
    floor: "二楼检查室",
  ...
  },
  {
    ...
    icon: "费",
    color: "#b57918",
    scene: "费用室桌上放着四本账：出门前的预防账和评价账，交付前的内部失效账，交付后的外部失效账。",
    mnemonic: "按时间记四本账：预防、评价、内部失效、外部失效。",
  ...
  },
  {
    ...
    icon: "设",
    color: "#5b4aa8",
    scene: "三楼手术室有两张床：设计床上放坏黏胶和好磁铁，编码床上放 const 护具和 NULL 牌。",
    mnemonic: "设计撕坏黏胶、增强好磁铁；编码给指针戴 const 护具。",
  ...
  },
  {
    ...
    floor: "四楼检测中心",
    label: "测试流水线",
    short: "模块台 接口桥 整机房 验收门",
    icon: "测",
    color: "#467b42",
    scene: "四楼是一条检测流水线：模块台、接口桥、整机房、用户验收门。旁边一人拿放大镜找错，另一人拿扳手改错。",
    mnemonic: "沿流水线走：模块、接口、整体、用户。",
  ...
  },
  {
    ...
    floor: "五楼白盒实验室",
    label: "白盒实验室",
    short: "接口 数据 边界 路径 错误处理",
    icon: "白",
    color: "#2f7597",
    scene: "透明盒子里所有电线都看得见。桌上贴着五张完整标签：接口、数据、边界、路径、错误处理。旁边还有六盏逻辑覆盖灯。",
    mnemonic: "白盒不要背缩写，直接看五张标签和六盏灯。",
  ...
  },
  {
    ...
    icon: "黑",
    color: "#2f3f4f",
    scene: "黑盒子只露出输入口和输出口。门口先分有效队和无效队，再看门槛边界，最后把输入原因和输出结果拉线。",
    mnemonic: "黑盒三动作：分队伍、看门槛、拉因果线。",
    ...
    prompt: "先说等价类和边界值，再说因果图、判定表、错误推测。若问桩/驱动，回到“上驱下桩”的小模型。"
  }
];
...
const drills = [
  ...
  {
    q: "质量管理体系怎么答？",
    hint: "总控室：方针旗、目标靶、策划图、职责牌、资源仓、产品线、测量仪、分析镜、改进轮。",
  ...
  {
    q: "IDEAL 模型五步是什么？",
    hint: "像医院处理流程：先开始，再诊断，再建方案，再行动，最后学习。",
    a: "初始化、诊断、建立、行动、学习。"
  },
  {
    q: "基本测量原则至少要抓住哪些关键词？",
    hint: "量尺八件套：理论地基、目标靶、清楚刻度、直觉灯、计算器、剪刀自动机、统计桥、可靠锁、反馈喇叭。",
